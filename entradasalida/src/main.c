...
int main(int argc, char* argv[]) {
    ...
    void* bits = calloc(1, block_count/8);
    ...
    if (bit_size < block_count/8) fwrite(bits + bit_size, block_count/8 - bit_size, 1, fd_bitmap);
    ...
    for (op_code cod_op = recibir_operacion(kernel_fd); cod_op != -1; cod_op = recibir_operacion(kernel_fd)){
        if (cod_op == IO){
            ...
            if(io->type_instruction == IO_FS_CREATE) {
                ...
                void* bits = calloc(1, block_count/8);
                fread(bits, block_count/8, 1, fd_bitmap);
                fseek(fd_bitmap, 0, SEEK_SET);
                mem_hexdump(bits, block_count/8);

                t_bitarray* bit_map = bitarray_create_with_mode(bits, block_count/8, MSB_FIRST);
                int posicion = -1;
                ...
                fwrite(bits, block_count/8, 1, fd_bitmap);
                fseek(fd_bitmap, 0, SEEK_SET);
