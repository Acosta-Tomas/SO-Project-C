...
void* interrupt_main(void *arg){
    log_debug(logger, "Thread interrupt_main creado");
    ...
    log_info(logger, "SOCKET: %d - CPU Interrupt", conexion);
    ...
    while (1){
        ...
        log_debug(logger, "Enviada Interrupcion PID: %u", interrupt_pid->pid);
    }
...
void* quantum_thread(void* q){
    ...
    log_debug(logger, "Comienzo Quantum - PID: %u - Tiempo : %u", pid_quantum->pid, pid_quantum->quantum);
