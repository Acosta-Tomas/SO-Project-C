...
int main(int argc, char* argv[]) {
    ...
    logger = log_create(LOGS_FILE, config_get_string_value(config, KEY_LOGGER), false, log_level_from_string(config_get_string_value(config, KEY_LOG_LEVEL)));
...
void cambio_estado(t_pcb* pcb, pid_status new_status){
    log_info(logger, "PID: %u - Estado Anterior: %s - Estado Actual: %s", pcb->pid, map_status(pcb->status), map_status(new_status));

    pcb->status = new_status;
}


char* map_status(pid_status status) {
    switch (status){
        case NEW: return "NEW";
        case READY: return "READY";
        case RUNNING: return "RUNNING";
        case RUNNING_QUANTUM: return "RUNNING";
        case RUNNING_SIGNAL: return "RUNNING";
        case BLOCKED_IO: return "BLOCKED";
        case BLOCKED_WAIT: return "BLOCKED";
